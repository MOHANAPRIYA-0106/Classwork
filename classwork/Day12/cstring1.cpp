...
int main()
{
	...
	strcpy(s3, s1);					//s3=s1+s2


	ptr=strcpy(s3, s2);				//s3=s3+s2

	cout<<s3<<endl;

	cout<<"ptr:"<<ptr<<endl;	     //ptr=s3

	char *ptr2 = strcat(ptr, s2);

	cout<<s3<<endl;						//s3=s3+s2

	cout<<" ptr2 "<<strcat (ptr2) <<endl;	//ptr2=s3

	strcat(s3, s2);					//s3=s3+s2
	...
	int res = strcmp(s1, s2);				//res=s1-s2
